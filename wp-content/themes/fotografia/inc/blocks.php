		...
		foreach ($blocks as $block) {
			...
			if (file_exists($scriptsfolder)) {
				...
				foreach ($scripts as $script) {
					...
					if (substr($script_filename, 0, 1) === '_')
						continue;
			...
			if ($block_settings["category"] == "fotografia_hero")
...
/**
 * Add a block category for all fotografia ACF blocks.
 *
 */
 ...
 */
add_filter('allowed_block_types_all', 'fotografia_allowed_block_types');
function fotografia_allowed_block_types($allowed_blocks)
{
	global $customblocks;
	$ret = array(
		'core/paragraph',
 ...
 *
 */
function fotografia_block_editor_styles()
{
	wp_enqueue_style('fotografia-editor-styles', get_theme_file_uri('/css/editor.css'), [], '1.0', 'all');
}
add_action('admin_enqueue_scripts', 'fotografia_block_editor_styles');
...
add_action('fotografia_pre_render_block', 'pre_render_block_cb', 10, 1);
function pre_render_block_cb($block)
{
	...
	global $breakpoints;
	if (
		(isset($block['data']['spacing_desktop']) && $block['data']['spacing_desktop'] != '') or
		(isset($block['data']['spacing_tablet']) && $block['data']['spacing_desktop'] != '') or
		(isset($block['data']['spacing_mobile']) && $block['data']['spacing_desktop'] != '')
	) {

		$style = '<style>';

		if (isset($block['data']['spacing_desktop']) && $block['data']['spacing_desktop'] != '')
			$style .= '@media only screen and (min-width: ' . $breakpoints['desktop-s'] . ' ) { #' . $block['id'] . '{ margin-bottom: ' . $block['data']['spacing_desktop'] . 'px; } }';

		if (isset($block['data']['spacing_tablet']) && $block['data']['spacing_tablet'] != '')
		...
		if (isset($block['data']['spacing_mobile']) && $block['data']['spacing_mobile'] != '')
	...
	if (isset($block['data']['_is_preview'])) {
		...
		if (is_admin() && file_exists($imagepath)) {
			?>
			<figure>
